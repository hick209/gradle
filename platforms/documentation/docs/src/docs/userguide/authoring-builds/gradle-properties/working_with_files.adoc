...
The exception are archive tasks, see <<sec:reproducible_archives,Reproducible archives are enabled by default>> for details.
...
== Reproducible archives are enabled by default
...
[[sec:archives_permissions]]
== Setting archive file permissions
...
For the first case, please see <<#sec:revert_reproducible_archives,Preserve file system-defined aspects>> section.
For the second case, permissions can be adjusted for `AbstractArchiveTask` and subclasses in the same way as for `CopySpec`, see <<#sec:setting_file_permissions,Setting file permissions>> for details.
